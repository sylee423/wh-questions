    ...
    for dt in ['singleton', 'set_id']:
        ...
        for br in base_rates:
            if res := run_model(n_vials, br, dt):
                ...
                results.conditions[dt]['avoid_p'].append(res['avoid_contam']['p_which_uncont'])
...
exp2 = run_exp2(n_vials=5)
```

```{python}
#| label: fig-exp2
#| fig-cap: "Experiment 2: Goal effect is strong for singleton but reduced for set identification"

fig, axes = plt.subplots(1, 2, figsize=(10, 4))
br = np.array(exp2.base_rates)

for ax, (dt, title) in zip(axes, [('singleton', 'Singleton selection'), ('set_id', 'Set identification (F1 score)')]):
    find_y = exp2.conditions[dt]['find_p']
    avoid_y = exp2.conditions[dt]['avoid_p']

    ax.plot(br, find_y, 'o-', color=COLORS['find'], linewidth=2, markersize=6, label='find_uncontam')
    ax.plot(br, avoid_y, 'o-', color=COLORS['avoid'], linewidth=2, markersize=6, label='avoid_contam')
...
plt.tight_layout()
plt.savefig(SCRIPT_DIR / 'exp2_figure.pdf', format='pdf', bbox_inches='tight')
plt.show()
print(f"Saved: {SCRIPT_DIR / 'exp2_figure.pdf'}")
```
